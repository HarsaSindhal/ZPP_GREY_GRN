...
Loop AT respo-tabledata into data(wa_respo).
 ...
 DATA(MAT) = get_mat( mat = wa_respo-material ).
 ...
 FROM I_MATERIALdocumentitem_2
      WHERE BATCH = @WA_RESPO-recbatch AND Plant = @respo-plant AND Material = @MAT AND GoodsMovementType IN ( '101','102' )
...
IF productionorder is NOT INITIAL.
      MODIFY ENTITIES OF i_materialdocumenttp
                ...
                FROM VALUE #(   (
                                ...
                                %target = VALUE #( FOR any IN respo-tabledata INDEX INTO i ( %cid                           = |My%CID_{ i }_001|
                                                     plant                          = respo-plant
                                                     material                       = get_mat( mat = any-material )              "'YGI1Z14N02009040'
 ...
 IF commit_failed IS NOT INITIAL.
     IF commit_reported-materialdocumentitem IS NOT INITIAL.

      DATA(msz) = commit_reported-materialdocumentitem[ 1 ]-%msg->if_message~get_text( ).
      DATA(msz_1) = | { msz } |.
      DATA(mszty) = 'E' .

      ELSEIF commit_reported-materialdocument IS NOT INITIAL.

      msz = commit_reported-materialdocument[ 1 ]-%msg->if_message~get_text( ).
      msz_1 = | { msz } |.
      mszty = 'E' .
...
WZPPGREY-Material = get_mat( mat = <FS>-material )  .
      ...
      MODIFY ENTITIES OF i_materialdocumenttp
                ...
                FROM VALUE #(   (
                                ...
                                %target = VALUE #( FOR any1 IN respo-tabledata1 INDEX INTO i ( %cid                           = |My%CID_{ i }_001|
                                                     plant                          = respo-plant
                                                     material                       = get_mat( mat = any1-material )               "'YGI1Z14N02009040'
...
IF commit_failed2 IS NOT INITIAL.
     IF commit_reported2-materialdocumentitem IS NOT INITIAL.

      DATA(msz2) = commit_reported2-materialdocumentitem[ 1 ]-%msg->if_message~get_text( ).
      DATA(msz_12) = | { msz2 } |.
      DATA(mszty2) = 'E' .

      ELSEIF commit_reported2-materialdocument IS NOT INITIAL.

      msz2 = commit_reported2-materialdocument[ 1 ]-%msg->if_message~get_text( ).
      msz_12 = | { msz2 } |.
      mszty2 = 'E' .
...
IF result IS NOT INITIAL AND result1 IS NOT INITIAL AND ( result21 IS INITIAL OR result21 = '0000' ).
 ...
 LOOP AT respo-tabledata ASSIGNING FIELD-SYMBOL(<GSS1>) .
  ...
  mat = get_mat( mat = <GSS1>-material ).

              UPDATE zpp_grey_grn_tab SET error261  = @result2  WHERE
              plant = @RESPO-plant AND  material = @mat AND recbatch = @<GSS1>-recbatch AND
...
mat = <fs1>-material.
              ...
              plant = @RESPO-plant AND  material = @mat AND materialdocument101 = @result AND
